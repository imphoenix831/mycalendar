...
import static net.imphoenix.mycalendar.Constant.getNowTimeString;
import static net.imphoenix.mycalendar.RcActivity.alSch;
import static net.imphoenix.mycalendar.RcActivity.alType;
...
public class DBUtil {
    ...
    { try
        { //建立或打開資料庫, 若資料庫不存在, 就建立一個新資料庫
            ...
            alType.clear();  //清除 alType ArrayList 集合
            while(cursor.moveToNext())   //判斷是否有下一個
            {
                alType.add(cursor.getString(1));  //得到資料存入 alType
            }
            sld.close();
            cursor.close();
        } catch(Exception e)
    ...
    public static void loadSchedule (RcActivity father ) //從日程數據庫讀取日程數據
    {
        ...
        { sld=SQLiteDatabase.openDatabase
            ...
            while (cursor.moveToNext())
            {
                ...
                alSch.add(schTemp);  //將 schTemp 存入 alSch 集合中

                Log.d("schdata",""+cursor.getPosition()+":sn="+sn+":"+date1+","+time1+","+date2+","+time2+","+timeSet);
    ...
    public static void insertSchedule(RcActivity father) //插入日程
    {
        ...
        catch (Exception e)
        {
            ...
            Log.d("ins Schedule Exception",e.toString());
        }
    ...
    public static void deletePassedSchedule(RcActivity father)
    {
        try {
            sld = SQLiteDatabase.openDatabase(
                    "/data/data/net.imphoenix.mycalendar/mydb",
