...
import static net.imphoenix.mycalendar.Constant.DIALOG_ABOUT;
import static net.imphoenix.mycalendar.Constant.DIALOG_ALL_DEL_CONFIRM;
import static net.imphoenix.mycalendar.Constant.DIALOG_CHECK;
import static net.imphoenix.mycalendar.Constant.DIALOG_SCH_DEL_CONFIRM;
import static net.imphoenix.mycalendar.Constant.DIALOG_SET_DATETIME;
import static net.imphoenix.mycalendar.Constant.DIALOG_SET_SEARCH_RANGE;
import static net.imphoenix.mycalendar.Constant.getNowDateString;
import static net.imphoenix.mycalendar.DBUtil.deletePassedSchedule;
import static net.imphoenix.mycalendar.DBUtil.deleteType;
import static net.imphoenix.mycalendar.DBUtil.deleteschedule;
import static net.imphoenix.mycalendar.DBUtil.getAllType;
import static net.imphoenix.mycalendar.DBUtil.insertSchedule;
import static net.imphoenix.mycalendar.DBUtil.insertType;
import static net.imphoenix.mycalendar.DBUtil.loadSchedule;
import static net.imphoenix.mycalendar.DBUtil.loadType;
import static net.imphoenix.mycalendar.DBUtil.searchSchedule;
import static net.imphoenix.mycalendar.DBUtil.updateSchedule;
...
public class RcActivity extends AppCompatActivity {
    ...
    ArrayList<Boolean> alIsSelected=new ArrayList<Boolean>();//記錄ListView中哪項選中了的標誌位
    ...
    ImageButton bEdit, bCheck , bDel, bNew, bDelAll, bSearch;
    ListView lv;

    Constant.WhoCall wcSetTimeOrAlarm;//用來判斷調用時間日期對話框的按鈕是設置時間還是設置鬧鐘,以便更改對話框中的一些控件該設置為visible還是gone
    Constant.WhoCall wcNewOrEdit;//用來判斷調用日程編輯界面的是新建日程按鈕還是在修改日程按鈕，以便設置對應的界面標題

    int sel=0;
    Constant.myLayout curr = null;//記錄當前界面的枚舉類型
    ...
    {
        @Override public void handleMessage(Message msg) //重寫 handleMessage 方法
        {
            switch(msg.what) //判斷收到消息
            {
                case 0:  //如果收到消息為 0
                    gotoMain();   //跳轉主程式
                    break;
    ...
    public void goToWelcomeView() {
        MySurfaceView myview=new MySurfaceView(this);   //建立 MySurfaceView的物件
        getWindow().setFlags(
                WindowManager.LayoutParams.FLAG_FULLSCREEN,
                WindowManager.LayoutParams.FLAG_FULLSCREEN) ; //設置為全螢幕
        setContentView(myview);  //進入 MySurfaceView 介面
        curr= Constant.myLayout.WELCOME_VIEW;
        gotoMain();
    }
    ...
    public void gotoMain()  //初始化主界面
    {
        ...
        setContentView(R.layout.main);  //進入主介面

        curr = Constant.myLayout.MAIN;

        sel = 0 ;

        lv =(ListView)findViewById(R.id.lvmainSchedule);//日程列表

        bEdit=(ImageButton)findViewById(R.id.ibmainEdit);//修改日程按鈕
        bCheck=(ImageButton)findViewById(R.id.ibmainCheck);//查看日程詳細內容的按鈕
        bDel=(ImageButton)findViewById(R.id.ibmainDel);//刪除當前選中日程的按鈕
        bNew=(ImageButton)findViewById(R.id.ibmainNew);//新建日程按鈕
        bDelAll=(ImageButton)findViewById(R.id.ibmainDelAll);//刪除所有過期日程按鈕
        bSearch=(ImageButton)findViewById(R.id.ibmainSearch);//查找日程按鈕
        ...
        //以下是ListView設置
        //建立自訂的 ListView Adapter
        myAdapter adapter = new myAdapter(this);
        lv.setAdapter(adapter);
        lv.setOnItemClickListener(lvListener);
        bNew.setOnClickListener(bNewListener);  //新增行程
        bEdit.setOnClickListener(bEditListener); //編輯行程
        bDel.setOnClickListener(bDelListener);   //刪除單一筆行程
        bDelAll.setOnClickListener(bDelAllListener);  //刪除所有過期日程按鈕
        bSearch.setOnClickListener(bSearchListener);   //日程查找按鈕
    ...
    public class myAdapter extends BaseAdapter {
        ...
        public myAdapter (Context c){
            ...
            //ListView 中時間 TextView
            TextView tvTime = new TextView(RcActivity.this);
            tvTime.setText(alSch.get(position).getTime1());
    ...
    //定義 onItemClick
    private ListView.OnItemClickListener lvListener =
        ...
        @Override public void onItemClick(AdapterView<?> parent, View view , int position, long id )
            {
                ...
                schTemp = alSch.get(position);  //選中該項目時，把該項目對像賦給schTemp
    ...
    private ImageButton.OnClickListener bNewListener = (
            new ImageButton.OnClickListener() {
                ...
                public void onClick(View view) {
                    ...
                    wcNewOrEdit = Constant.WhoCall.NEW;  //調用日程編輯界面的是新建按鈕
                    gotoSetting();  //去日程編輯界面
                }
            }
          );
    ...
    //bEdit設置
    public Button.OnClickListener bEditListener =
            new Button.OnClickListener() {
                @Override public void onClick(View v) {
                    wcNewOrEdit = Constant.WhoCall.EDIT;
                    gotoSetting(); //去日程編輯界面
                }
            };

    //刪除選中的日程按鈕
    public Button.OnClickListener bDelListener =
            new Button.OnClickListener() {
                @Override public void onClick(View v) {
    ...
    //刪除所有過期日程按鈕
    public Button.OnClickListener bDelAllListener =
            new Button.OnClickListener() {
                @Override public void onClick(View v) {
                    showDialog(DIALOG_ALL_DEL_CONFIRM);
                }
            }; // end of bDelAllListener

    //	日程查找按鈕
    public Button.OnClickListener bSearchListener =
            new Button.OnClickListener() {
                @Override public void onClick(View v) {
                    gotoSearch();
                }
    ...
    {  //畫面 newSchedule.xml
        ...
        curr = Constant.myLayout.SETTING;
        ...
        // 類型 spinner設置
        myTypeAdapter spTypeAdapter = new myTypeAdapter(this);
        spType.setAdapter(spTypeAdapter);
        spType.setSelection(sel);

        bNewType.setOnClickListener(bNewTypeListener);  //新建日程類型按鈕
        bSetDate.setOnClickListener(bSetDateListener);   //設置日期與時間
        bSetAlarm.setOnClickListener(bSetAlarmListener);  //設置 鬧鉒 日期與時間

        bDone.setOnClickListener(bDoneListener); //按下完成按鈕
    ...
    public class myTypeAdapter extends BaseAdapter {
            ...
            public myTypeAdapter(Context c) {
                myInflater = LayoutInflater.from(c);
            }
        ...
        } // end of  myTypeAdapter

    public Button.OnClickListener bNewTypeListener =
                new Button.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        try {
                            //將已經輸入的title和note存入schTemp，以防返回時被清空
                            final EditText etTitle = (EditText) findViewById(R.id.etxjrcTitle);
                            final EditText etNote = (EditText) findViewById(R.id.etxjrcNote);
                            final Spinner spType = (Spinner) findViewById(R.id.spxjrcType);
    ...
    public Button.OnClickListener bSetDateListener =
            new Button.OnClickListener() {
                @Override public void onClick(View v) {
                    EditText etTitle = (EditText) findViewById(R.id.etxjrcTitle);
                    EditText etNote = (EditText) findViewById(R.id.etxjrcNote);
                    Spinner spType = (Spinner) findViewById(R.id.spxjrcType);

                    schTemp.setTitle(etTitle.getText().toString()); //將已經輸入的title和note存入schTemp，以防返回時被清空
                    schTemp.setNote(etNote.getText().toString());
                    sel=spType.getSelectedItemPosition();//存儲spType的當前選擇
                    wcSetTimeOrAlarm = Constant.WhoCall.SETTING_DATE; //調用設置日期時間對話框的是設置日程日期按鈕
                    showDialog(DIALOG_SET_DATETIME);

                }
            } ; // end of bSetDataListener

    private Button.OnClickListener bSetAlarmListener =
            new Button.OnClickListener() {
                @Override public void onClick(View v) {
                    EditText etTitle = (EditText) findViewById(R.id.etxjrcTitle);
                    EditText etNote = (EditText) findViewById(R.id.etxjrcNote);
                    ...
                    wcSetTimeOrAlarm = Constant.WhoCall.SETTING_ALARM;  //調用設置日期時間對話框的是設置鬧鐘按鈕
                    showDialog(DIALOG_SET_DATETIME);
                }
            }; // end of bSetAlarmListener

    private Button.OnClickListener bDoneListener =
            new Button.OnClickListener() {
               @Override public void onClick(View v) {
                   EditText etTitle = (EditText) findViewById(R.id.etxjrcTitle);
                   EditText etNote = (EditText) findViewById(R.id.etxjrcNote);
                   ...
                   if (schTemp.isPassed()) {
                       Toast.makeText(RcActivity.this, "不能建立過期日程", Toast.LENGTH_LONG).show();
                       return;
                   }
                   ...
                   if (wcNewOrEdit == Constant.WhoCall.NEW) //如果當前界面是新建日程，調用插入日程方法
                   {
                       insertSchedule(RcActivity.this);
                   }
                   else if (wcNewOrEdit == Constant.WhoCall.EDIT) //如果當前界面是修改日程，調用更新日程方法
                   {
                       updateSchedule(RcActivity.this);
                   }
                   gotoMain();
               }
            }; //end of bDoneListener


    //===================================查找界面start=========================================
    public void gotoSearch()
    {   setContentView(R.layout.search);
        curr= Constant.myLayout.SEARCH;
...
*/
    ...
    { Dialog dialog=null;
      ...
      {   case DIALOG_SET_SEARCH_RANGE:
          ...
          case DIALOG_SCH_DEL_CONFIRM: //按下刪除按鈕
          ...
          case DIALOG_ALL_DEL_CONFIRM: //按下刪除所有行程按鈕
              AlertDialog.Builder abSchAllDelConfirm = new AlertDialog.Builder(this);
              abSchAllDelConfirm.setItems(null,null);
              dialogAllDelConfirm = abSchAllDelConfirm.create();
   ...
   // 每次彈出Dialog對話框時更新對話框的內容
    @Override public void onPrepareDialog(int id,Dialog dialog )
    {
...
//year month day後面是1的表示關於起始時間設置，2表示關於終止時間設置，P表示plus加號，M表示minus建號
            ...
            if(wcSetTimeOrAlarm == Constant.WhoCall.SETTING_DATE)  //如果是設置日期按鈕調用的本對話框
            {
            ...
            //如果調用該界面的是設置鬧鐘按鈕
            if(wcSetTimeOrAlarm==Constant.WhoCall.SETTING_ALARM)
            {
                        ...
                        (
                                new View.OnClickListener()
                                {
                                    ...
                                    public void onClick(View v)
                                    {
                                        ...
                                        if(wcSetTimeOrAlarm==Constant.WhoCall.SETTING_DATE)
                                        {
                                        ...
                                        else if(wcSetTimeOrAlarm==Constant.WhoCall.SETTING_ALARM)
                                        {
    ...
    public void gotoSearchResult()//該界面和主界面除了少了幾個按鈕其他完全一樣
    {
        ...
        curr= Constant.myLayout.SEARCH_RESULT;
        ...
        bEdit.setOnClickListener
                (
                        new View.OnClickListener()
                        {
                            ...
                            @Override
                            public void onClick(View v) {
                                wcSetTimeOrAlarm=Constant.WhoCall.EDIT;
                                gotoSetting();
                            }
                        }
                );
                        ...
                        new BaseAdapter()
                        {
                            ...
                            public View getView(final int position, View convertView, ViewGroup parent)
                            {
                                ...
                                if(position>=defultType.length)
                                {
                                    ...
                                    ib.setLayoutParams(new ViewGroup.LayoutParams(24, 24));
