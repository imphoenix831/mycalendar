...
import static net.imphoenix.mycalendar.DBUtil.getSNFromPrefs;
...
public class Schedule
{
    ...
    {
        sn = 0 ;
        date1 = toDateString(y,m,d);   //資料轉換為日期
        time1 = toTimeString(8,0) ; //時間預設 8 點
        date2 = null;  //鬧鐘日期
        time2 = null;  //鬧鐘時間
        title = "" ;   //日程標題
        note = "" ; //日程備註
        type = ""; //日程類型
        timeSet = true;  //日程是否設置具體時間
        alarmSet = false; //日程是否設置鬧鐘
    }
    ...
    public static String toDateString (int y,int m , int d  ) { //靜態方法，把int型的年月日轉換成YYYY/MM/DD
        StringBuffer sb =new StringBuffer();   //建立 StringBuffer 物件
        sb.append(y);
        sb.append("/");
        sb.append(m<10?"0"+m:""+m);  //判斷 m 是否小於 10
        sb.append("/");
        sb.append(d<10?"0"+d:""+d);  //判斷 d 是否小於 10
        return sb.toString();
    }
